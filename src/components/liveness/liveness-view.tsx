...
import type { AadhaarVerification, LivenessChallenge, AadhaarLivenessStep } from '@/types';
import {
...
function ChallengeIcon({ type, animated }: { type: LivenessChallenge['type']; animated: boolean }) {
...
function CameraViewfinder({ captured }: { captured: boolean }) {
  return (
    <div className="relative w-full aspect-[4/3] bg-zinc-950 rounded-xl overflow-hidden border border-zinc-800">
      {/* Corner brackets */}
      ...
      {/* Animated scan line */}
      {!captured && (
        <motion.div className="absolute left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-emerald-400 to-transparent"
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        {captured ? (
          ...
          <motion.div animate={{ opacity: [0.3, 0.6, 0.3] }} transition={{ duration: 2, repeat: Infinity }}
            className="w-28 h-36 rounded-full border-2 border-dashed border-zinc-600" />
        )}
      </div>
      {/* Recording indicator */}
...
// ---- Status Badge ----
function StatusBadge({ status }: { status: AadhaarVerification['status'] }) {
  const config: Record<string, { label: string; variant: 'default' | 'secondary' | 'destructive' | 'outline'; className: string }> = {
    verified: { label: 'Verified', variant: 'default', className: 'bg-emerald-500/15 text-emerald-600 border-emerald-500/30' },
    verifying: { label: 'Verifying', variant: 'secondary', className: 'bg-amber-500/15 text-amber-600 border-amber-500/30' },
    mismatch: { label: 'Mismatch', variant: 'destructive', className: 'bg-red-500/15 text-red-600 border-red-500/30' },
    failed: { label: 'Failed', variant: 'destructive', className: 'bg-red-500/15 text-red-600 border-red-500/30' },
...
// ---- Demo Steps Config ----
const DEMO_CHALLENGES: LivenessChallenge[] = [
...
const STEPS: { key: AadhaarLivenessStep; label: string; icon: React.ReactNode }[] = [
  ...
  { key: 'aadhaar_verify', label: 'Aadhaar Verify', icon: <Shield className="w-4 h-4" /> },
...
export function LivenessView() {
  const [verifications] = useState<AadhaarVerification[]>(() => generateAadhaarVerifications());
  const [statusFilter, setStatusFilter] = useState<string>('all');
  const [expandedRow, setExpandedRow] = useState<string | null>(null);

  // Demo state
  const [demoRunning, setDemoRunning] = useState(false);
  const [demoStep, setDemoStep] = useState<AadhaarLivenessStep>('photo_capture');
  const [demoChallenges, setDemoChallenges] = useState<LivenessChallenge[]>(DEMO_CHALLENGES.map(c => ({ ...c })));
  const [photoCaptured, setPhotoCaptured] = useState(false);
  const [faceMatchScore, setFaceMatchScore] = useState(0);
  const [antiSpoofScore, setAntiSpoofScore] = useState(0);
  const [aadhaarMatchScore, setAadhaarMatchScore] = useState(0);
  const [aadhaarDataMatch, setAadhaarDataMatch] = useState({ name: false, dob: false, gender: false, photo: false, address: false });
  ...
  const demoResult: 'verified' | 'mismatch' | 'failed' | null =
    demoStep === 'result' && demoRunning
      ? (aadhaarDataMatch.photo && faceMatchScore > 80 ? 'verified' : 'mismatch')
      : null;
  ...
  const resetDemo = useCallback(() => {
    ...
    setFaceMatchScore(0);
    setAntiSpoofScore(0);
    setAadhaarMatchScore(0);
    setAadhaarDataMatch({ name: false, dob: false, gender: false, photo: false, address: false });
  }, []);

  const advanceDemo = useCallback((step: AadhaarLivenessStep, delay: number) => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      else advanceDemo('aadhaar_verify', 1200);
    };
    requestAnimationFrame(animate);
  }, [demoRunning, demoStep, advanceDemo]);

  // Aadhaar verify step animation
  useEffect(() => {
    if (!demoRunning || demoStep !== 'aadhaar_verify') return;
    const fields = ['name', 'dob', 'gender', 'photo', 'address'] as const;
    fields.forEach((field, i) => {
      setTimeout(() => {
        setAadhaarDataMatch(prev => ({ ...prev, [field]: field !== 'address' ? true : Math.random() > 0.3 }));
      }, 600 * (i + 1));
    });
    setTimeout(() => {
      setAadhaarMatchScore(+(88 + Math.random() * 8).toFixed(1));
      setDemoStep('result');
    }, 600 * (fields.length + 1));
  }, [demoRunning, demoStep]);

  // Result is now derived from aadhaarDataMatch and faceMatchScore (see demoResult const above)
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="flex items-start gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Aadhaar Liveness Detection</h1>
          <p className="text-muted-foreground text-sm mt-1">
            Anti-spoofing identity verification with biometric liveness challenges and Aadhaar data matching
          </p>
        </div>
        <Badge className="ml-auto bg-emerald-500/15 text-emerald-600 border-emerald-500/30">
          <ShieldCheck className="w-3 h-3 mr-1" /> ISO 42001 Certified
        </Badge>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}
        ...
        {[
          ...
          <motion.div key={stat.label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            ...
            <Card className="bg-card/60 backdrop-blur-sm border-border/50 hover:shadow-lg transition-shadow">
        ...
        <Card className="bg-card/60 backdrop-blur-sm border-border/50">
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>Step-by-step Aadhaar liveness verification simulation</CardDescription>
              </div>
              ...
              {STEPS.map((step, i) => (
                <div key={step.key} className="flex items-center flex-1">
                  <div className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium transition-all duration-300 ${
                    i < currentStepIndex ? 'bg-emerald-500/15 text-emerald-600' :
                    i === currentStepIndex ? 'bg-emerald-500/25 text-emerald-600 ring-1 ring-emerald-500/40' :
                  ...
                  {i < STEPS.length - 1 && (
                    <div className={`flex-1 h-0.5 mx-1 rounded transition-colors duration-300 ${
                      i < currentStepIndex ? 'bg-emerald-500/50' : 'bg-muted/30'}`} />
                  )}
                </div>
              ))}
            </div>
          ...
          <CardContent>
            <AnimatePresence mode="wait">
                ...
                <motion.div key="photo_capture" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    <div className="space-y-3">
                      <CameraViewfinder captured={photoCaptured} />
                      <Button className="w-full" onClick={() => setPhotoCaptured(true)}
                        disabled={photoCaptured || !demoRunning}>
                      ...
                      <motion.div key={i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
                        ...
                        className={`p-4 rounded-xl border transition-all duration-300 ${
                          challenge.completed
                            ...
                            : 'bg-card border-border/50 ring-1 ring-amber-500/30'
                        }`}>
                        ...
                        {challenge.completed ? (
                          <div className="space-y-2">
                            <div className="flex items-center justify-between text-xs">
                              ...
                              <span className="font-mono">{challenge.confidence.toFixed(1)}%</span>
                            </div>
                ...
                <motion.div key="face_match" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
                  ...
                  <div className="grid md:grid-cols-3 gap-6 items-center">
                    ...
                    <div className="text-center space-y-2">
                      <div className="w-32 h-40 mx-auto rounded-xl bg-zinc-900 border-2 border-emerald-500/30 flex items-center justify-center">
                    ...
                    {/* Aadhaar Photo */}
                    <div className="text-center space-y-2">
                      <div className="w-32 h-40 mx-auto rounded-xl bg-zinc-900 border-2 border-sky-500/30 flex items-center justify-center">
                        <div className="text-center">
                          <ShieldUser className="w-10 h-10 text-sky-400/50 mx-auto mb-1" />
                          <span className="text-xs text-zinc-500">Aadhaar Photo</span>
                        </div>
                      </div>
              ...
              {demoStep === 'aadhaar_verify' && (
                <motion.div key="aadhaar_verify" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
                  exit={{ opacity: 0, x: -20 }} className="space-y-4">
                  <h3 className="font-semibold flex items-center gap-2">
                    <Shield className="w-4 h-4 text-emerald-500" /> Aadhaar Data Verification
                  </h3>
                  <p className="text-sm text-muted-foreground">
                    Cross-referencing captured data against UIDAI Aadhaar database records.
                  </p>
                  <div className="grid sm:grid-cols-2 gap-3">
                    {[
                      ...
                      const checked = aadhaarDataMatch[field.key];
                      return (
                        <motion.div key={field.key} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }}
                          transition={{ delay: i * 0.1 }}
                          className={`flex items-center gap-3 p-3 rounded-lg border transition-all duration-300 ${
                            checked ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-muted/30 border-border/50'}`}>
                  ...
                  <div className="flex items-center justify-between p-3 rounded-lg bg-muted/30">
                    <span className="text-sm text-muted-foreground">Aadhaar Match Score</span>
                    <span className="text-sm font-bold font-mono">{aadhaarMatchScore.toFixed(1)}%</span>
                  </div>
                </motion.div>
              )}
                  ...
                  <div className={`p-6 rounded-2xl border-2 ${
                    ...
                    <div className="text-center space-y-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground mt-1">
                          {demoResult === 'verified'
                            ? 'All biometric checks passed. Aadhaar data matches.'
                    ...
                    <div className="grid grid-cols-3 gap-4 mt-6">
                      {[
                        ...
                        { label: 'Aadhaar Match', value: aadhaarMatchScore, icon: <Fingerprint className="w-4 h-4" /> },
                      ].map(score => (
                        <div key={score.label} className="text-center p-3 rounded-xl bg-background/50">
                    ...
                    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
                      className={`mt-4 p-3 rounded-lg flex items-center gap-3 ${
                        demoResult === 'verified' ? 'bg-emerald-500/10' : 'bg-red-500/10'}`}>
                      {demoResult === 'verified' ? (
                  ...
                  <div className="flex gap-1.5">
                    {['all', 'verified', 'mismatch', 'failed', 'verifying'].map(filter => (
                      <Button key={filter} size="sm" variant={statusFilter === filter ? 'default' : 'outline'}
                        ...
                        className={statusFilter === filter ? 'bg-emerald-600 hover:bg-emerald-700' : ''}>
                      ...
                      {filteredVerifications.map(v => {
                        ...
                        return (
                          <TableRow key={v.id} className="cursor-pointer hover:bg-muted/30"
                            ...
                            <TableCell className="font-mono text-xs text-muted-foreground">{v.aadhaarNumber}</TableCell>
                  ...
                  {expandedRow && (() => {
                    ...
                    return (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}
                        ...
                        className="mt-4 p-4 rounded-xl bg-muted/30 border border-border/50 space-y-3 overflow-hidden">
                        <h4 className="text-sm font-semibold flex items-center gap-2">
                          <ZoomIn className="w-4 h-4" /> Data Match Details — {v.candidateName}
                        </h4>
                        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                          {Object.entries(v.aadhaarDataMatch).map(([field, matched]) => (
                        ...
                        <div className="grid sm:grid-cols-3 gap-3 pt-2 border-t border-border/50">
                          <div className="text-center p-2 rounded-lg bg-background/50">
                            ...
                            <p className={`text-lg font-bold font-mono ${v.faceMatchScore > 80 ? 'text-emerald-600' : 'text-red-600'}`}>
                              {v.faceMatchScore.toFixed(1)}%
                            </p>
                          </div>
                          <div className="text-center p-2 rounded-lg bg-background/50">
                            <p className="text-xs text-muted-foreground">Anti-Spoof</p>
                            <p className={`text-lg font-bold font-mono ${v.antiSpoofScore > 90 ? 'text-emerald-600' : 'text-amber-600'}`}>
                              {v.antiSpoofScore.toFixed(1)}%
                            </p>
                          </div>
                          <div className="text-center p-2 rounded-lg bg-background/50">
                            <p className="text-xs text-muted-foreground">Aadhaar Match</p>
                            <p className={`text-lg font-bold font-mono ${v.aadhaarMatchScore > 80 ? 'text-emerald-600' : 'text-amber-600'}`}>
                              {v.aadhaarMatchScore.toFixed(1)}%
                            </p>
                          </div>
                        </div>
                        <div>
                          ...
                          <div className="flex gap-2 flex-wrap">
                            {v.challenges.map((c, i) => (
                              <div key={i} className={`px-3 py-1.5 rounded-lg text-xs flex items-center gap-1.5 ${
                                ...
                                <span className="font-mono">{c.confidence.toFixed(1)}%</span>
                              </div>
                            ))}
                          </div>
                        </div>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {SPOOF_TYPES.map((spoof, i) => (
                <motion.div key={spoof.type} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <Card className="bg-card/60 backdrop-blur-sm border-border/50 hover:shadow-lg transition-shadow h-full">
                    <CardContent className="p-4 space-y-3">
                      ...
                      <div className="space-y-1.5">
                        <div className="flex items-center justify-between text-xs">
                          ...
                          <span className="font-mono font-medium">{spoof.accuracy}%</span>
                        </div>
